...
router.get('/',auth,async(req,res) => {
    try {
        const posts = await Post.find().sort({date: -1}); // date: -1 -=> the first one will be the recently added post
        res.json(posts);
    } catch (error) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.delete('/comment/:id/:comment_id',auth, async (req, res) => {
    try {
        ...
        //pull out comment
        const comment = post.comments.find(comment => comment.id === req.params.comment.id);
