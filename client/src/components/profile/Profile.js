...
const Profile = ({getProfileById, profile: {profile,loading}, auth, match}) => {
    ...
    return (
        <Fragment>{profile === null || loading ? <Spinner/> : <Fragment>
                <Link to='/profiles' className='btn btn-light'>
                    Back To Profiles
                </Link>
                {auth.isAuthenticate && auth.loading === false && auth.users._id === profile.user._id && (<Link to = '/edit-profile' className='btn btn-dark'>Edit Profile</Link>)}
